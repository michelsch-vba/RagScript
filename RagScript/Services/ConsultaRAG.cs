...
namespace RagScript.Services
{
    public class MotorBuscaRAG
    {
            ...
            float threshold = 0.50f)
        {
            ...
            // 1. Aluguel seguro via ArrayPool em vez de stackalloc (evita StackOverflowException)
            float[] bufferPergunta = ArrayPool<float>.Shared.Rent(tamanhoVetor);
            Span<float> perguntaSpan = bufferPergunta.AsSpan(0, tamanhoVetor);

            float[] bufferChunk = ArrayPool<float>.Shared.Rent(tamanhoVetor);
            Span<float> chunkFloatSpan = bufferChunk.AsSpan(0, tamanhoVetor);
            ...
            try
            {
                ...
                // Tabela alinhada com o SqliteVectorRepository (Chunks)
            ...
            {
                // Devolução rigorosa dos buffers para a POO do ArrayPool
                ArrayPool<float>.Shared.Return(bufferPergunta);
                ArrayPool<float>.Shared.Return(bufferChunk);
            }
        ...
        public List<string> ObterSugestoesMembros()
        {
            ...
            FROM Chunks 
            WHERE (NomeMembro IS NOT NULL AND NomeMembro != '') OR (NomeArquivo IS NOT NULL AND NomeArquivo != '');";
        ...
        public Dictionary<string, string> ObterHashesPorCaminho()
        {
            ...
            string sql = "SELECT DISTINCT CaminhoRelativo, HashConteudo FROM Chunks;";
            using var comando = new SqliteCommand(sql, conexao);
            using var reader = comando.ExecuteReader();
        ...
        {
            // Para comandos de modificação (DELETE), abrimos a conexão sem o modo ReadOnly
            string conexaoEscrita = _conexaoSqlite.Replace(";Mode=ReadOnly;", "");
            using var conexao = new SqliteConnection(conexaoEscrita);
            await conexao.OpenAsync();

            string sql = "DELETE FROM Chunks WHERE CaminhoRelativo = $caminho;";
            using var comando = new SqliteCommand(sql, conexao);
